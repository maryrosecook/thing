...
;(function(exports) {
  ...
  exports.Snowflake = {
    ...
    every: function(interval, action, arg) {
      ...
      {
        this.run(action, arg);
        actions[id].nextRun = this.calculateNextRun(interval);
      }
    },
    ...
    watchedValueAction: function(watchValues, watchValueTest, action, arg) {
      var id = this.id(action);
      if(actions[id] === undefined)
        actions[id] = { watchValues: watchValues };
      else
      {
        if(watchValueTest(actions[id].watchValues, watchValues) === true)
          this.run(action, arg);
